...
contract MultiSigWalletMetaTxnWithVerifier is ERC2771Context {
    ...
    function executeTransaction(
        ...
        uint256[2] memory c,
        uint256[1] memory input
    ) public verifyProofModifier(a, b, c, input) txExists(_txIndex) notExecuted(_txIndex) {
        Transaction storage transaction = transactions[_txIndex];
    ...
    function getTransactionCount(
        ...
        uint256[2] memory c,
        uint256[1] memory input
    ) public view verifyProofModifier(a, b, c, input) returns (uint256) {
